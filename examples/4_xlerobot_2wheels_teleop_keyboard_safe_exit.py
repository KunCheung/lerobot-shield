...
def safe_log_rerun(observation=None, action=None):
    try:
        ...
        print(f"[RERUN] Non-fatal logging error: {exc}")


def _compute_target_error(obs, joint_map, target_positions):
    errors = []
    for joint_name, target in target_positions.items():
        motor_name = joint_map[joint_name]
        current = obs.get(f"{motor_name}.pos", target)
        errors.append(abs(current - target))
    return max(errors, default=0.0)


def _compute_head_target_error(obs, head_control):
    errors = []
    for motor_name, target in head_control.target_positions.items():
        current = obs.get(f"{HEAD_MOTOR_MAP[motor_name]}.pos", target)
        errors.append(abs(current - target))
    return max(errors, default=0.0)
...
def _safe_disconnect(device, name):
    try:
        if getattr(device, "is_connected", False):
            device.disconnect()
            print(f"[SHUTDOWN] {name} disconnected")
    except Exception as exc:
...
def controlled_shutdown(robot, keyboard, left_arm, right_arm, head_control, reason, fps=SHUTDOWN_CONTROL_FPS):
    ...
    try:
        if not robot.is_connected:
            print("[SHUTDOWN] Robot already disconnected")
            return

        smooth_controller.current_speed = 0.0
        ...
        while time.time() < deadline:
            action = {
                **left_arm.p_control_action(robot),
                **right_arm.p_control_action(robot),
                **head_control.p_control_action(robot),
                "x.vel": 0.0,
                "theta.vel": 0.0,
            }
            robot.send_action(action)
            ...
            try:
                ...
                print(f"[SHUTDOWN] Non-fatal base stop error: {exc}")

            obs = robot.get_observation()
            left_error = _compute_target_error(obs, LEFT_JOINT_MAP, left_arm.target_positions)
            right_error = _compute_target_error(obs, RIGHT_JOINT_MAP, right_arm.target_positions)
            head_error = _compute_head_target_error(obs, head_control)
            max_error = max(left_error, right_error, head_error)

            if max_error <= SHUTDOWN_POSITION_TOLERANCE:
                print(f"[SHUTDOWN] Safe pose reached with max error {max_error:.2f}")
                break

            precise_sleep(1.0 / fps)
        else:
            print(
                f"[SHUTDOWN] Timed out after {SHUTDOWN_SETTLE_TIMEOUT_S:.1f}s while moving to safe pose; disconnecting anyway"
            )

        try:
            robot.send_action({"x.vel": 0.0, "theta.vel": 0.0})
        except Exception as exc:
            print(f"[SHUTDOWN] Non-fatal final base zeroing error: {exc}")

    except Exception as exc:
        print(f"[SHUTDOWN] Controlled shutdown encountered a non-fatal error: {exc}")
    finally:
        _safe_disconnect(robot, "robot")
        _safe_disconnect(keyboard, "keyboard")
...
def main():
    ...
    try:
        init_rerun(session_name="xlerobot_2wheels_teleop_safe_exit")
    except Exception as exc:
        print(f"[RERUN] Visualization unavailable: {exc}")
    ...
    try:
        while True:
            try:
                ...
                shutdown_reason = "keyboard listener disconnected (likely ESC pressed)"
                break
